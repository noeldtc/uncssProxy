...
$( document ).ready(function() {
    ...
    $.ajax({
        type: "GET",
        url: "https://www.kimovil.com/es/comparar-moviles/f_max_d+eurPrice.450,f_not_m+code.CN:IN#localprices", 
        headers: {
        ...
        success: function(response) {
            ...
            let listDevices = body.getElementsByClassName('item-wrap');
            for(let i=0; i < listDevices.length;i++){
                let tr = document.createElement("tr");
                let dev = listDevices[i];

                let title = dev.getElementsByClassName("device-name")[0].getElementsByClassName("title")[0];
                let tdTitle = document.createElement("td");
                tdTitle.innerHTML = title.innerHTML;

                let rank = dev.getElementsByClassName("objetive-wrap")[0].getElementsByClassName("rank")[0];
                let tdRank = document.createElement("td");
                tdRank.innerHTML = rank.innerHTML;


                let tdPrice = document.createElement("td");
                getPrice(tdPrice,dev.getElementsByClassName('device-data')[0].getElementsByClassName("price-wrap")[0].href)
